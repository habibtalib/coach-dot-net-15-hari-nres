...
2. **Lengkapkan ujian** dengan `qa-uat` (UJI-01):
   ...
   ```text
   Guna subagent qa-uat: lengkapkan ujian xUnit — peralihan SubmissionStatus
   (Draft→Submitted→AdminApproved/Rejected + peralihan terlarang mesti gagal),
   nombor rujukan LD berjujukan, dan semakan pendua dua arah. Guna SQLite in-memory
   (bukan UseInMemoryDatabase). Jalankan dotnet test, tunjuk keputusan.
   ```

3. **Pratonton pre-check pelayar** (penuh Hari 15) — app rujukan di `https://localhost:7034`:

   ```text
   Guna subagent qa-uat: pre-check di https://localhost:7034 — log masuk applicant,
   OfficerReporting → hantar permohonan (catat LD-…), log masuk hradmin → Review → Luluskan
   → sahkan audit. Sahkan applicant ke /OfficerReporting/Review mesti 403. Jangan cetuskan dialog.
   ```

### ✅ Semakan

- [ ] `qa-uat` + `/uji-modul` wujud; `dotnet test` hijau
- [ ] Peralihan status, nombor rujukan LD, semakan pendua dilindungi
