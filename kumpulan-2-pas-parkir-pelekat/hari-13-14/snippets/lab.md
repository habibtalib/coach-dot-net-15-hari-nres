...
2. **Lengkapkan ujian** dengan `qa-uat` (UJI-01):
   ...
   ```text
   Guna subagent qa-uat: lengkapkan ujian xUnit — normalisasi nombor plat,
   semakan pendua plat dua arah, dan peruntukan lot. Guna SQLite in-memory
   (bukan UseInMemoryDatabase). Jalankan dotnet test, tunjuk keputusan.
   ```

3. **Pre-check pelayar** menggantikan langkah manual Latihan 5 — jalankan app repo `pas-parkir-pelekat` anda:

   ```text
   Guna subagent qa-uat: pre-check di app pas-parkir-pelekat — log masuk applicant →
   mohon pas/pelekat, log masuk SecurityAdmin → luluskan. Sahkan applicant ke halaman
   semakan mesti 403. Rekod lulus/gagal + GIF. Jangan cetuskan dialog.
   ```

### ✅ Semakan

- [ ] `qa-uat` + `/uji-modul` wujud; `dotnet test` hijau
- [ ] Normalisasi plat, pendua dua arah, peruntukan lot dilindungi
