...
## Latihan 6 — Pemecut QA dipandu-AI (Claude Code)

**Objektif:** Guna subagent `qa-uat` + skill `/uji-modul` untuk mempercepat ujian blok ini, dan memandu semakan RBAC 403 (teras security audit K3) melalui pelayar. Lab penuh: [`lab-qa-ai-uat.md`](../../../docs/lab-qa-ai-uat.md).
...
2. **Lengkapkan ujian** dengan `qa-uat` (UJI-01):
   ...
   ```text
   Guna subagent qa-uat: lengkapkan ujian xUnit — laluan kelulusan dua peringkat
   (Supervisor→IctAdmin), penjanaan nama akaun AD, dan kelulusan separa. Guna SQLite
   in-memory (bukan UseInMemoryDatabase). Jalankan dotnet test, tunjuk keputusan.
   ```

3. **Semakan RBAC 403 dipandu-AI** (UJI-03) — jalankan app repo `id-ad-email` anda:

   ```text
   Guna subagent qa-uat: sebagai peranan salah, cuba setiap halaman semakan admin
   (kelulusan peringkat 1 & 2). Setiap satu mesti 403/AccessDenied. Senaraikan hasil setiap semakan.
   ```

> **Ingat:** modul ini **tidak** menyimpan kata laluan sebenar — titik pengajaran keselamatan. Jangan minta AI menambahnya.

### ✅ Semakan

- [ ] `qa-uat` + `/uji-modul` wujud; `dotnet test` hijau
- [ ] Kelulusan dua peringkat, nama akaun AD, kelulusan separa dilindungi
- [ ] Setiap halaman semakan → **403** untuk peranan salah (masuk ke `security-audit.md`)
