...
| A11 | Tempah fasiliti sukan | Applicant | TFS-2026-#### dijana | |
| A12 | Cuba tempah slot bertindih | Applicant | Ditolak — pertindihan slot | |
| A13 | Luluskan tempahan | FacilityAdmin | Status Completed, slot diperuntukkan | |
...
| B3 | Carian global "TFS-2026" | Menemui tempahan fasiliti | |
...
## Latihan 6b — SIT/UAT pre-check dipandu-AI (claude-in-chrome)

**Objektif:** Pandu SESI 46 pre-check secara automatik dengan subagent `qa-uat` + MCP **claude-in-chrome** — log masuk, isi borang, klik, baca halaman — dan rekod lulus/gagal. Lab penuh: [`../../docs/lab-qa-ai-uat.md`](../../docs/lab-qa-ai-uat.md) (Latihan 4).

> 🔧 **Khusus Claude Code.** Guna alat AI lain? Langkau — Latihan 6 (skrip rentas modul manual) sudah memadai.

### Langkah

1. Cipta (sekali) `.claude/agents/qa-uat.md` + `.claude/skills/uji-modul/SKILL.md` (salin templat kursus). Pastikan app bersepadu berjalan (`https://localhost:7034`) dan Chrome dibenarkan untuk `localhost`.
2. **Pandu aliran hujung-ke-hujung (Bahagian A)** dengan `qa-uat`:

   ```text
   Guna subagent qa-uat: jalankan SIT/UAT pre-check di https://localhost:7034.
   Buka tab baharu, kemudian: log masuk staf baharu → Lapor Diri (profil dicipta) →
   guna aliran modul dalaman melalui SSO. Rekod lulus/gagal setiap langkah + GIF.
   Jangan cetuskan sebarang dialog/alert.
   ```

3. **Semakan RBAC (Bahagian C) — tujuh 403:**

   ```text
   Guna subagent qa-uat: untuk setiap halaman semakan admin merentas modul, cuba
   akses dengan peranan SALAH. Kesemua tujuh mesti 403/AccessDenied. Senaraikan hasil.
   ```

4. Rekod keputusan dalam skrip SIT anda (sama seperti Latihan 6).

> ⚠️ **Pre-check ≠ UAT sebenar.** Ia mengesahkan aliran & sempadan. **UAT sebenar dijalankan pengguna NRES** terhadap keperluan mereka — nyatakan dalam serahan.

### ✅ Semakan

- [ ] `qa-uat` memandu aliran hujung-ke-hujung; lulus/gagal direkod
- [ ] Kesemua **tujuh 403** disahkan melalui pelayar (padan Bahagian C Latihan 6)
- [ ] Keputusan ditanda **pre-check**, bukan UAT sebenar
